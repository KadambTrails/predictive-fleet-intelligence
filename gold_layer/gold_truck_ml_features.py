...
# Weather Encoding - Forced to .lower() using the dbt 'weather' column output
df['is_weather_clear'] = df['weather'].apply(lambda x: 1 if str(x).lower() == 'clear' else 0)
df['is_weather_hail'] = df['weather'].apply(lambda x: 1 if str(x).lower() == 'hailstorm' else 0)
df['is_weather_rain'] = df['weather'].apply(lambda x: 1 if str(x).lower() == 'heavy rain' or str(x).lower() == 'rain' else 0)
...
# Target Labels (Labels for our Algos)
...
# 5. Filter for final ML Matrix columns matching ClickHouse exactly
ml_columns = [
    'truck_id',
    'event_timestamp',
    'engine_temp',
    ...
    'from_austin',
    'to_charlotte',
    'label_maintenance_required',
    'label_is_anomaly'
]

# 6. Insert the Data
ch_client.insert_df('truck_logistics_gold.gold_truck_ml_features', df[ml_columns])
